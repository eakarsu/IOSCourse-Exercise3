...
executeScript(`
  ...
  CREATE TABLE IF NOT EXISTS runtime_sessions(
    token_hash TEXT PRIMARY KEY,user_id UUID NOT NULL REFERENCES runtime_users(id) ON DELETE CASCADE,expires_at TIMESTAMPTZ NOT NULL,created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
  );
  CREATE TABLE IF NOT EXISTS runtime_ai_interactions(
    id BIGSERIAL PRIMARY KEY,user_id UUID NOT NULL REFERENCES runtime_users(id),input JSONB NOT NULL,output JSONB NOT NULL,
    model TEXT NOT NULL,created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
  );
  CREATE INDEX IF NOT EXISTS runtime_ai_user_idx ON runtime_ai_interactions(user_id,created_at DESC);
